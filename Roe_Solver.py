...
while(t_sim<t):
    ...
    for i in range(n-1):
        ...
        a_til=((gamma-1)*(H_til-u_til**2/2))**0.5
        ...
        delta[0]=(1/(2*a_til))*((rho_R-rho_L)*(u_til+a_til) \
                              - (rho_R*u_R-rho_L*u_L) - a_til*delta[1])
    ...
    for i in range(n):
        ...
        F_cell[i]=np.array([rho_upd*u_upd,rho_upd*u_upd**2 \
                            + P_upd,u_upd*(rho_upd*e_upd+P_upd)])
...
ax[0].scatter(x_plot,rho_plot,marker='.',label='Roe solver')
ax[0].set_xlabel('Position')
ax[0].set_ylabel('Density')
ax[0].set_title("Density profile (Roe's method)")
ax[0].legend()

ax[1].plot(x_std,P_std,'k',label='Analytic')
ax[1].scatter(x_plot,P_plot,marker='.',label='Roe solver')
ax[1].set_xlabel('Position')
ax[1].set_ylabel('Pressure')
ax[1].set_title("Pressure profile (Roe's method)")
ax[1].legend()

ax[2].plot(x_std,u_std,'k',label='Analytic')
ax[2].scatter(x_plot,u_plot,marker='.',label='Roe solver')
ax[2].set_xlabel('Position')
ax[2].set_ylabel('Velocity')
ax[2].set_title("Velocity profile (Roe's method)")
ax[2].legend()

ax[3].plot(x_std,energy_std,'k',label='Analytic')
ax[3].scatter(x_plot,energy_plot,marker='.',label='Roe solver')
ax[3].set_xlabel('Position')
ax[3].set_ylabel('Energy')
ax[3].set_title("Energy profile (Roe's method)")
ax[3].legend()
